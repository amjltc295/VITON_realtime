""" Real-time demo for VITON test stage 1.
...
def _process_image(image_name, product_image_name, sess,
    ...
    segment_raw = sio.loadmat(os.path.join(FLAGS.segment_dir,
                                           image_id))["segment"]
...
def main(unused_argv):
    if not os.path.exists(FLAGS.result_dir):
        ...
        print("Directory {} made".format(FLAGS.result_dir))
    result_images_dir = FLAGS.result_dir + 'images/'
    if not os.path.exists(result_images_dir):
        os.makedirs(result_images_dir)
        print("Directory {} made".format(result_images_dir))
    result_tps_dir = FLAGS.result_dir + 'tps/'
    if not os.path.exists(result_tps_dir):
        os.makedirs(result_tps_dir)
        print("Directory {} made".format(result_tps_dir))
    ...
    with tf.Session() as sess:
        ...
        for i in range(FLAGS.begin, FLAGS.end, batch_size):
            ...
            for j in range(batch_size):
                ...
                scipy.misc.imshow(image_output[j])
                input()
